...
<p:declare-step xmlns:p="http://www.w3.org/ns/xproc" xmlns:c="http://www.w3.org/ns/xproc-step"
    ...
    <p:variable name="folder_path" select="'./'"/>
    ...
    <p:for-each name="files" message="Load files in alphabetical order">
        ...
        <!--<p:insert match="*:body/*:entryFree[1]/(node()|text())[1]" position="first-child" message="Inserting pb to
                {count(//*:body/*)}">-->
        <!--<p:with-option name="match" select="'*:body/*:entryFree[1]'"/>-->
        <p:insert position="first-child" message="Inserting pb to {count(//*:body/*)} with
            {$current-file}">
            <p:with-option name="match"
                select="'*:body/*:entryFree[1]/*[not(self::*:milestone)][1]'"/>
            <!--<p:with-option name="match"
                select="if (./*:body/node()[1][self::text()][normalize-space()])
                then '*:body'
                else '*:body/*[not(self::*:milestone)][1]'"/>-->
    ...
    <!-- <p:insert match="//*:body" position="first-child" message="Inserting pb to {c:file}">
        <p:with-input port="insertion">
            <pb n="{c:file}"/>
        </p:with-input>
    </p:insert>
